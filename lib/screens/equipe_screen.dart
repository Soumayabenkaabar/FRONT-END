...
class _EquipeScreenState extends State<EquipeScreen> {
  ...
  @override
  void initState() { super.initState(); loadMembres(); }
  ...
  Future<void> showAddMembreDialog() async {
    ...
    final projets        = projetsRaw.map((p) => {'id': p.id, 'titre': p.titre}).toList();
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(builder: (context, sd) {
        return _MembreDialog(
          ...
          nomCtrl: nomCtrl, roleCtrl: roleCtrl,
          specialiteCtrl: specialiteCtrl, emailCtrl: emailCtrl, telCtrl: telCtrl,
          disponible: disponible,
          onDisponibleChanged: (v) => sd(() => disponible = v),
          projets: projets, selectedProjetId: selectedProjetId,
          onProjetChanged: (v) => sd(() => selectedProjetId = v),
          btnLabel: 'Ajouter le membre', btnIcon: LucideIcons.userPlus,
          onSubmit: () async {
            ...
            await MembreService.addMembre(Membre(
              ...
              projetsAssignes: disponible ? [] : [
                projets.firstWhere((p) => p['id'].toString() == selectedProjetId)['titre']!
              ],
            ));
            Navigator.pop(context); loadMembres();
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading) return const Center(child: CircularProgressIndicator(color: kAccent));
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          IntrinsicHeight(child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Expanded(child: _StatCard(title: 'En activité', value: '${actifs.length}',      icon: LucideIcons.activity,    color: const Color(0xFFD97706))),
            const SizedBox(width: 10),
              ...
              for (int i = 0; i < disponibles.length; i += cols) {
                ...
                rows.add(IntrinsicHeight(child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  for (int j = 0; j < rowItems.length; j++) ...[
                    ...
                    Expanded(child: MembreDisponibleCard(membre: rowItems[j],
                  ...
                  for (int k = rowItems.length; k < cols; k++) ...[const SizedBox(width: 16), const Expanded(child: SizedBox())],
                ])));
          ...
          if (actifs.isNotEmpty) ...[
            _SectionTitle(icon: LucideIcons.briefcase, color: kAccent, label: 'Membres en activité'),
            const SizedBox(height: 14),
          ...
          if (filtered.isEmpty) Center(child: Padding(
            padding: const EdgeInsets.symmetric(vertical: 40),
            child: Column(children: const [
              Icon(LucideIcons.users, size: 40, color: kTextSub),
              SizedBox(height: 12),
              Text('Aucun membre trouvé', style: TextStyle(color: kTextSub, fontSize: 15, fontWeight: FontWeight.w500)),
            ]),
          )),
        ]),
      ),
      ...
      return _MembreDialog(
        ...
        onSubmit: () async {
          ...
          await MembreService.updateMembre(Membre(
            id: membre.id,
            nom: nomCtrl.text, role: roleCtrl.text,
            specialite: specialiteCtrl.text, email: emailCtrl.text,
            telephone: telCtrl.text, disponible: disponible,
            projetsAssignes: disponible
                ? []
                : [projets.firstWhere((p) => p['id'] == selectedProjetId)['titre']!],
          ));
...
void showAssignDialog(BuildContext context, Membre membre, VoidCallback onRefresh) async {
  ...
  showDialog(context: context, builder: (_) => StatefulBuilder(builder: (context, sd) {
    return Dialog(
          ...
          Container(
            ...
            child: Row(children: [
              Container(
                ...
                child: const Icon(LucideIcons.folderInput, color: Colors.white, size: 20),
              ),
              ...
              Container(
                ...
                child: Row(children: [
                  Container(width: 40, height: 40,
                    ...
                    child: const Icon(LucideIcons.user, color: kAccent, size: 18)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(membre.role, style: const TextStyle(color: kTextSub, fontSize: 12)),
                  ])),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: const Color(0xFFDCFCE7), borderRadius: BorderRadius.circular(20)),
                    child: const Text('Disponible', style: TextStyle(color: Color(0xFF16A34A), fontSize: 11, fontWeight: FontWeight.w700)),
                  ),
                ]),
              ),
              const SizedBox(height: 18),
              const Text('SÉLECTIONNER UN PROJET',
                  style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: kTextSub, letterSpacing: 0.8)),
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: isSelected ? kAccent : const Color(0xFFE5E7EB), width: isSelected ? 2 : 1),
                    ),
                    child: Row(children: [
                      Container(width: 10, height: 10,
                        decoration: BoxDecoration(color: isSelected ? kAccent : const Color(0xFFD1D5DB), shape: BoxShape.circle)),
                      const SizedBox(width: 12),
                      Expanded(child: Text(p['titre'] ?? '',
                          style: TextStyle(fontSize: 13, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w400, color: isSelected ? kAccent : kTextMain))),
                      if (isSelected) const Icon(LucideIcons.check, size: 16, color: kAccent),
                    ]),
                  ),
                );
              }),
            ...
            child: Row(children: [
              ...
              Expanded(child: ElevatedButton.icon(
                onPressed: selectedId == null ? null : () async {
                  final proj = projets.firstWhere((p) => p['id'] == selectedId);
                  await MembreService.assignMembre(membre: membre, projet: proj['titre']!);
                  Navigator.pop(context);
                  _showSnack(context, '${membre.nom} assigné avec succès', kAccent);
                  onRefresh();
                },
                icon: const Icon(LucideIcons.check, size: 15, color: Colors.white),
                label: const Text('Confirmer', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                style: ElevatedButton.styleFrom(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Container(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(membre.nom, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17, color: Colors.white), overflow: TextOverflow.ellipsis),
              const SizedBox(height: 4),
        ...
        Padding(
          ...
          child: Column(children: [
            _InfoTile(icon: LucideIcons.settings2, label: 'Spécialité', value: membre.specialite, color: kAccent),
            ...
            if (membre.projetsAssignes.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Container(width: 6, height: 6, decoration: BoxDecoration(color: kAccent, borderRadius: BorderRadius.circular(3))),
                    const SizedBox(width: 8),
                    Text('${membre.projetsAssignes.length} projet(s) assigné(s)',
                  ...
                  ...membre.projetsAssignes.map((p) => Padding(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text(p, style: const TextStyle(fontSize: 13, color: kTextMain, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
                    ]),
                  )),
                ]),
              ),
...
class _MembreDialog extends StatelessWidget {
  ...
  final TextEditingController nomCtrl, roleCtrl, specialiteCtrl, emailCtrl, telCtrl;
  ...
  const _MembreDialog({
    ...
    required this.icon, required this.btnLabel, required this.btnIcon,
    this.iconBg, this.iconColor, this.btnColor,
    required this.nomCtrl, required this.roleCtrl,
    required this.specialiteCtrl, required this.emailCtrl, required this.telCtrl,
    required this.disponible, required this.onDisponibleChanged,
    required this.onSubmit,
    this.projets, this.selectedProjetId, this.onProjetChanged,
  });
          ...
          Padding(
            ...
            child: Column(children: [
              ...
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                decoration: BoxDecoration(
                  color: disponible ? const Color(0xFFF0FDF4) : const Color(0xFFFFFBEB),
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(
                      color: disponible ? const Color(0xFFBBF7D0) : kAccent.withOpacity(0.4)),
                ),
                child: Row(children: [
                  Icon(disponible ? LucideIcons.checkCircle : LucideIcons.briefcase,
                      color: disponible ? const Color(0xFF16A34A) : kAccent, size: 18),
                  const SizedBox(width: 10),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Statut : ${disponible ? "Disponible" : "En activité"}',
                        style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13,
                            color: disponible ? const Color(0xFF16A34A) : kAccent)),
                    const SizedBox(height: 2),
                    const Text('Basculer pour changer le statut',
              ...
              if (!disponible && projets != null) ...[
                ...
                Container(
                  ...
                  child: DropdownButtonHideUnderline(child: DropdownButton<String>(
                    ...
                    isExpanded: true,
                    icon: const Icon(LucideIcons.chevronsUpDown, size: 16, color: kAccent),
                    dropdownColor: Colors.white,
                    items: projets!.map<DropdownMenuItem<String>>((p) => DropdownMenuItem(
                      value: p['id'],
                      child: Row(children: [
                ...
                if (selectedProjetId != null) ...[
                  const SizedBox(height: 8),
                  Row(children: [
                    const Icon(LucideIcons.info, size: 13, color: kAccent),
                    const SizedBox(width: 6),
                    Expanded(child: Text(
                      'Projet actuel chargé depuis la base de données',
                      style: TextStyle(fontSize: 11, color: kAccent.withOpacity(0.8), fontStyle: FontStyle.italic),
                    )),
                  ]),
                ],
              ],
            ...
            child: Row(children: [
              ...
              Expanded(child: ElevatedButton.icon(
                ...
                label: Text(btnLabel,
                    ...
                    overflow: TextOverflow.ellipsis),
                style: ElevatedButton.styleFrom(
                  backgroundColor: bColor, elevation: 0,
                  padding: const EdgeInsets.symmetric(vertical: 13),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                ),
              )),
...
class _DialogField extends StatelessWidget {
  ...
  const _DialogField({required this.icon, required this.label, required this.hint,
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    TextField(controller: controller, keyboardType: keyboardType,
      ...
      decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(color: kTextSub),
        prefixIcon: Icon(icon, size: 14, color: kTextSub), isDense: true,
