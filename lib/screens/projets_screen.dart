...
class _ProjetsScreenState extends State<ProjetsScreen> {
  ...
  Future<void> loadProjets() async {
    try {
      ...
      setState(() { projets = data; isLoading = false; });
    } catch (e) {
...
List<Membre> membres = [];
    ...
    showDialog(
      context: context,
      builder: (_) => StatefulBuilder(builder: (ctx, sd) {
        return Dialog(
                  ...
                  child: Row(children: [
                    ...
                    const Expanded(child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 3),
                        Text('Créez un nouveau projet de construction',
  ...
  child: Text(c.nom),
                    ...
                    Row(children: [
                      Expanded(child: _DropdownField(
                        ...
                        value: selectedChef,
                        items: membres.map<DropdownMenuItem<String>>((m) =>
DropdownMenuItem(
  value: m.nom,
  child: Text(m.nom),
)                        ).toList(),
...
switch (statut) {
  ...
  default:
    statutDb = 'en_cours';
                        ...
                        if (titreCtrl.text.trim().isEmpty) {
                          _showSnack(ctx, 'Le titre est obligatoire', kRed); return;
                        }
                        if (selectedClientId == null) {
                          _showSnack(ctx, 'Choisissez un client', kRed); return;
                        }
                        if (selectedChef == null) {
                          _showSnack(ctx, 'Choisissez un chef de projet', kRed); return;
                        }
                        final budgetVal = double.tryParse(budgetCtrl.text.replaceAll(' ', ''));
                        if (budgetCtrl.text.trim().isNotEmpty && budgetVal == null) {
                          _showSnack(ctx, 'Le budget doit être un nombre valide', kRed); return;
                        }
                        if (budgetVal != null && budgetVal < 0) {
                          _showSnack(ctx, 'Le budget ne peut pas être négatif', kRed); return;
                        }

                        sd(() => isSaving = true);
                        try {
                          final clientNom = (clients.firstWhere((c) => c.id == selectedClientId)).nom ;
                          final nouveau = Project(
                            ...
                            budgetTotal: double.tryParse(budgetCtrl.text.replaceAll(' ', '')) ?? 0,
                            budgetDepense: 0,
                            client: clientNom,
                          ...
                          loadProjets();
                          if (mounted) _showSnack(context, 'Projet créé avec succès', kAccent);
                        } catch (e) {
  ...
  _showSnack(ctx, 'Erreur: $e', kRed);
                      ...
                      icon: isSaving
                          ...
                          : const Icon(LucideIcons.folderPlus, size: 15, color: Colors.white),
                      label: Text(isSaving ? 'Création...' : 'Créer le projet',
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Mes Projets',
                  style: TextStyle(fontSize: isMobile ? 26 : 28,
            ...
            ElevatedButton.icon(
              ...
              label: Text(isMobile ? 'Nouveau' : 'Nouveau projet',
          ...
          IntrinsicHeight(child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Expanded(child: _MiniStat(label: 'Total',      value: '${projets.length}',
                icon: LucideIcons.layoutGrid, color: kAccent)),
            const SizedBox(width: 10),
            Expanded(child: _MiniStat(label: 'En cours',
                value: '${projets.where((p) => p.statut == "en_cours").length}',
                icon: LucideIcons.activity, color: const Color(0xFF3B82F6))),
            const SizedBox(width: 10),
            Expanded(child: _MiniStat(label: 'Terminés',
                value: '${projets.where((p) => p.statut == "termine").length}',
          ...
          SingleChildScrollView(
            ...
            child: Row(children: ['Tous', 'En cours', 'Planification', 'Terminé']
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          for (int j = 0; j < rowItems.length; j++) ...[
            ...
            Expanded(child: GestureDetector(
              onTap: () => Navigator.push(context, MaterialPageRoute(
                  builder: (_) => ProjetDetailScreen(project: rowItems[j], projectIndex: 0))),
              child: ProjectFullCard(project: rowItems[j]),
            )),
          ],
          for (int k = rowItems.length; k < columns; k++) ...[
            const SizedBox(width: 20), const Expanded(child: SizedBox()),
          ],
        ]),
      ));
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Text(label, style: const TextStyle(
          fontSize: 11, fontWeight: FontWeight.w700, color: kTextSub, letterSpacing: 0.5)),
      const SizedBox(height: 6),
      Container(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<String>(
            ...
            icon: const Icon(LucideIcons.chevronsUpDown, size: 14, color: kTextSub),
