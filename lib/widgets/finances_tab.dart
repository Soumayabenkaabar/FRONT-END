    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        LayoutBuilder(builder: (ctx, constraints) {
          ...
          final cards = [
            _FinKpiCard(icon: LucideIcons.dollarSign, iconColor: kAccent,
                label: 'Budget prévu',  value: _fmt(budget)),
            _FinKpiCard(icon: LucideIcons.trendingUp, iconColor: kAccent,
                label: 'Coût réalisé',  value: _fmt(depense),
                sub: '${(pctBudget * 100).toStringAsFixed(0)}% du budget'),
            _FinKpiCard(icon: LucideIcons.trendingDown, iconColor: const Color(0xFF10B981),
                label: 'Reste à payer', value: _fmt(restant)),
            _FinKpiCard(icon: LucideIcons.fileText, iconColor: kTextSub,
          ...
          return Column(children: [
            IntrinsicHeight(child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              if (cols == 4) ...[
                const SizedBox(width: 12), Expanded(child: cards[2]),
                const SizedBox(width: 12), Expanded(child: cards[3]),
              ],
            ])),
            if (cols == 2) ...[
        ...
        _Section(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(_fmt(depense), style: const TextStyle(color: kTextSub, fontSize: 11)),
              Text(_fmt(budget),  style: const TextStyle(color: kTextSub, fontSize: 11)),
            ]),
          ]),
        ),
        ...
        _Section(
          ...
          child: factures.isEmpty
              ? const Padding(
                  ...
                  child: Center(child: Text('Aucune facture pour ce projet',
                      style: TextStyle(color: kTextSub, fontSize: 13))))
              : Column(
                  children: factures.map((f) => _FactureRow(
                    facture: f,
                    fmt: _fmt,
...
class _FactureRow extends StatelessWidget {
  ...
  const _FactureRow({
    required this.facture, required this.fmt,
    required this.factureColor, required this.factureLabel,
    required this.onStatusChanged, required this.onDelete,
  });
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      Container(
        ...
        child: const Icon(LucideIcons.fileText, color: Color(0xFF3B82F6), size: 18),
      ),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(facture.numero,
            style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: kTextMain)),
        Text(fmt(facture.montant),
            style: const TextStyle(color: kTextSub, fontSize: 12)),
        if (facture.dateEcheance != null)
          Text('Échéance : ${facture.dateEcheance}',
      ...
      Material(
        ...
        child: PopupMenuButton<String>(
          ...
          child: Container(
            ...
            child: Text(factureLabel,
                style: TextStyle(color: factureColor, fontSize: 11, fontWeight: FontWeight.w700)),
          ),
        ),
      ),
...
class _FinKpiCard extends StatelessWidget {
  ...
  const _FinKpiCard({required this.icon, required this.iconColor,
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 10),
      Text(value,
          style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16, color: kTextMain),
          overflow: TextOverflow.ellipsis),
      if (sub != null) ...[
